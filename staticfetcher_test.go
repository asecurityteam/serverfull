	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("Static.Fetch() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if (err != nil) && tt.wantErr && tt.wantErrType != nil {
				errType := reflect.TypeOf(err)
				if errType != tt.wantErrType {
					t.Errorf("Static.Fetch() error = %v, wantErrType %v", errType, tt.wantErrType)
				}
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("Static.Fetch() = %v, want %v", got, tt.want)
			}
		})
	}
}
